...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    # Conv layers conv_dw_
    #DW1
    x = KL.TimeDistributed(KL.DepthwiseConv2D((3, 3), padding="same", depth_multiplier=1, strides=strides, use_bias=False), name="mmrcnn_conv_dw1")(x)
    x = KL.TimeDistributed(BatchNorm(axis = channel_axis), name='mmrcnn_conv_dw1_bn')(x, training=train_bn)
    x = KL.Activation(relu6, name='mmrcnn_conv_dw1_relu')(x)
    #PW1
    x = KL.TimeDistributed(KL.Conv2D(256, (1, 1), padding="same", use_bias=False, strides=(1, 1)), name="mmrcnn_conv_pw1")(x)
    x = KL.Activation(relu6, name='mmrcnn_conv_pw1_relu')(x)

    #DW2
    x = KL.TimeDistributed(KL.DepthwiseConv2D((3, 3), padding="same", depth_multiplier=1, strides=strides, use_bias=False), name="mmrcnn_conv_dw2")(x)
    x = KL.TimeDistributed(BatchNorm(axis = channel_axis), name='mmrcnn_conv_dw2_bn')(x, training=train_bn)
    x = KL.Activation(relu6, name='mmrcnn_conv_dw2_relu')(x)
    #PW2
    x = KL.TimeDistributed(KL.Conv2D(256, (1, 1), padding="same", use_bias=False, strides=(1, 1)), name="mmrcnn_conv_pw2")(x)
    x = KL.Activation(relu6, name='mmrcnn_conv_pw2_relu')(x)

    #DW3
    x = KL.TimeDistributed(KL.DepthwiseConv2D((3, 3), padding="same", depth_multiplier=1, strides=strides, use_bias=False), name="mmrcnn_conv_dw3")(x)
    x = KL.TimeDistributed(BatchNorm(axis = channel_axis), name='mmrcnn_conv_dw3_bn')(x, training=train_bn)
    x = KL.Activation(relu6, name='mmrcnn_conv_dw3_relu')(x)
    #PW3
    x = KL.TimeDistributed(KL.Conv2D(256, (1, 1), padding="same", use_bias=False, strides=(1, 1)), name="mmrcnn_conv_pw3")(x)
    x = KL.Activation(relu6, name='mmrcnn_conv_pw3_relu')(x)

    #DW4
    x = KL.TimeDistributed(KL.DepthwiseConv2D((3, 3), padding="same", depth_multiplier=1, strides=strides, use_bias=False), name="mmrcnn_conv_dw4")(x)
    x = KL.TimeDistributed(BatchNorm(axis = channel_axis), name='mmrcnn_conv_dw4_bn')(x, training=train_bn)
    x = KL.Activation(relu6, name='mmrcnn_conv_dw4_relu')(x)
    #PW4
    x = KL.TimeDistributed(KL.Conv2D(256, (1, 1), padding="same", use_bias=False, strides=(1, 1)), name="mmrcnn_conv_pw4")(x)
    x = KL.Activation(relu6, name='mmrcnn_conv_pw4_relu')(x)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        _, C2, C3, C4, C5 = mobilenet_graph(input_image, config.BACKBONE, alpha=1.0, depth_multiplier=1, train_bn=config.TRAIN_BN)
        # Top-down Layers
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        layer_regex = {
            ...
            "3+": r"(conv.*5.*)|(conv.*6.*)|(conv.*7.*)|(conv.*8.*)|(conv.*9.*)|(conv.*10.*)|(conv.*11.*)|(conv.*12.*)|(conv.*13.*)|(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)",
